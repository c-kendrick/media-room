-- Atomic, owner-only restore of a Media Room v1 export. Existing records that
-- are absent from the backup are deliberately left untouched.
...
begin
  ...
  loop
    ...
    on conflict (collection_id, section, name) do update set
      ...
      show_in_main_watchlist = case when shelf_entry ? 'showInMainWatchlist' then excluded.show_in_main_watchlist else public.shelves.show_in_main_watchlist end,
      main_watchlist_position = case when shelf_entry ? 'mainWatchlistPosition' then excluded.main_watchlist_position else public.shelves.main_watchlist_position end
  ...
  loop
    ...
    on conflict (collection_id, legacy_id) do update set
      ...
      star_rating = case when entry ? 'star_rating' then excluded.star_rating else public.media_items.star_rating end,
      owned = case when entry ? 'owned' then excluded.owned else public.media_items.owned end,

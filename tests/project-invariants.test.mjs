...
test('mobile shelves and two-row search results share three-card sizing without sacrificing collision safety', async () => {
  ...
  assert.match(mediaStyles, /\.media-page \.media-search-results \{[^}]*padding: 12px 4px 15px;/);
  assert.match(mediaStyles, /\.media-search-results \.search-results-grid \{[\s\S]*grid-auto-flow: column;[\s\S]*grid-auto-columns: var\(--mobile-media-card-width\);[\s\S]*overflow-x: auto;[\s\S]*overflow-y: hidden;/);
...
test('dialogs use browser history and shelf controls keep the requested phone layout', async () => {
  ...
  assert.match(styles, /@media\(max-width:580px\)[\s\S]*\.shelf-actions\{[^}]*grid-template-columns:max-content max-content max-content;justify-content:end!important;[^}]*column-gap:7px!important;row-gap:7px!important;[^}]*padding-left:0!important/);

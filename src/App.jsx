...
function BulkImportDialog({ section, shelves, onClose, onImport }) {
  ...
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const [enrichPosters, setEnrichPosters] = useState(section === 'screen');
  ...
  return <div className="modal-layer editor-layer"><form className="media-edit-dialog bulk-import-dialog" onSubmit={async (event) => { event.preventDefault(); if (!shelfId || !items.length || invalidRows.length) return; setSaving(true); setError(''); try { await onImport(shelfId, items, enrichPosters); } catch { setError('Nothing was imported. Check the format, apply the latest Supabase migration, and try again.'); setSaving(false); } }}>
    ...
    {section === 'screen' && <label className="bulk-enrichment-option"><input type="checkbox" checked={enrichPosters} onChange={(event) => setEnrichPosters(event.target.checked)} /><span><strong>Add confident TMDB posters</strong><small>Checks up to 50 items using the protected TMDB key; ambiguous matches are left blank.</small></span></label>}

...
function exportCollection(snapshot) {
  const payload = { exported_at: new Date().toISOString(), format: 'media-room/v1', collection: { id: snapshot.collectionId, title: snapshot.collectionTitle, owner_id: snapshot.ownerId, descriptions: snapshot.collectionDescriptions }, libraries: snapshot.libraries || [], shelves: snapshot.mediaShelves, media: snapshot.media };
...
export default function App() {
  ...
  const [selectedMediaNavigation, setSelectedMediaNavigation] = useState(null);
  ...
  const rememberedLibrary = useRef('');
  ...
  useEffect(() => {
    const viewCopiedShelf = (event) => {
      ...
      if (sharedMode) {
        window.location.assign(appSiteUrl());
        return;
      }
  ...
  const pendingWatchlistRequests = userHub?.watchlist_requests || [];
        ...
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            {authLoading ? <span className="account-state">Checking account…</span> : (
              <Button className={cls('account-button', account && 'signed-in-account')} icon={account ? undefined : LogIn} aria-label={account ? `Open account for ${personDisplayName(account.profile)}${pendingWatchlistRequests.length ? `, ${pendingWatchlistRequests.length} watchlist requests` : ''}` : 'Sign in'} onClick={() => setAccountOpen(true)}>
                {account ? <><UserAvatar person={account.profile} size="account" /><span className="account-desktop-label">{personDisplayName(account.profile)}{viewAsAdmin ? ' - Admin view' : ''}</span>{pendingWatchlistRequests.length > 0 && <b className="account-request-badge">{pendingWatchlistRequests.length}</b>}</> : <span className="account-desktop-label">Sign in</span>}
              </Button>
            )}
          </div>
        </header>
      ...
      {importDraft && <AddMediaDialog
        ...
        onSave={(item, shelfIds) => {
          ...
          const temporaryItem = {
            ...
            lists: shelfIds,
            list_positions: Object.fromEntries(shelfIds.map((id) => [id, OPTIMISTIC_APPEND_POSITION])),
            interests: [],
...
function MediaView({ data, loading = false, initialSection, onLoadSection, onLoadLibrary, onInvalidateLibrary, onEnsureSectionDetails, onSectionChange, onDataChange, notify, openMedia, canEdit, canReact, currentUserId, onReaction, isAdmin, accessToken, refresh, requestConfirmation, mainWatchlistTitle, mainWatchlistClubs, mainWatchlistClubId, onMainWatchlistClubChange, onExport, onStarRatingChange, ownCollection, loadCopyDestinations, onViewCopiedShelf, sourceOwnerName, shareToken }) {
  ...
  const creatingShelfRequestRef = useRef(false);
  ...
  const openBin = () => {
    ...
    void Promise.all([
      ...
      loadCollectionLibraries({ collectionId: data.collectionId, includeDeleted: true, fresh: true, accessToken }),
    ]).then(([, allLibraries]) => {
      setBinLibraries(allLibraries.filter((library) => library.deleted_at));
    }).catch((error) => {
      setBinError(error?.message || 'The complete Bin could not be loaded.');
  ...
  const exportCompleteBackup = async () => {
    ...
    try {
      exportCollection(await loadCompleteCollection({ details: true }));
      notify('Backup prepared.');
    } catch {
      notify('The complete collection could not be prepared for export.');
    } finally {
  ...
  const toggleMainWatchlistShelf = async (shelf, requestedEnabled = !shelf.showInMainWatchlist) => {
  ...
  const saveLibrary = async (values) => {
    ...
    if (libraryEditor?.id) {
      ...
      await onLoadLibrary(libraryEditor.id, { fresh: true });
  ...
  const requestLibraryDeletion = () => {
    ...
    const shelfCount = (data.mediaShelves || []).filter((shelf) => !shelf.deleted_at && shelf.library_id === currentLibrary.id).length;
    ...
    <div className={cls('page media-page', data.mainWatchlist ? 'main-watchlist-page' : 'collection-page')}>
      ...
      {!sectionLoading && !data.mainWatchlist && (canEdit || isAdmin) && <section className="collection-tools">
        ...
        <div className="collection-tool-actions">
          ...
          <Button className="quiet-button" icon={Download} disabled={exportingBackup} onClick={exportCompleteBackup}>{exportingBackup ? 'Preparing backupâ€¦' : 'Export backup'}</Button>
          ...
          {canEdit && <Button className="quiet-button bin-button" icon={Trash2} onClick={openBin}>Bin{binCount ? ` (${binCount})` : ''}</Button>}
          {canEdit && section === 'screen' && <><Button className="quiet-button" icon={Plus} onClick={() => setBulkImportType('film')}>Bulk Import {bulkImportTerminology('film', currentLibrary).heading}</Button><Button className="quiet-button" icon={Plus} onClick={() => setBulkImportType('television')}>Bulk Import {bulkImportTerminology('television', currentLibrary).heading}</Button></>}
          {canEdit && section === 'book' && <Button className="quiet-button" icon={Plus} onClick={() => setBulkImportType('book')}>Bulk Import {bulkImportTerminology('book', currentLibrary).heading}</Button>}
          {canEdit && section === 'game' && <Button className="quiet-button" icon={Plus} onClick={() => setBulkImportType('game')}>Bulk Import {bulkImportTerminology('game', currentLibrary).heading}</Button>}
        </div>
      </section>}
      {creatingShelf && <CreateShelfDialog section={section} terminology={currentLibrary} onClose={() => setCreatingShelf(false)} onSave={async (values) => {
        if (creatingShelfRequestRef.current) return;
        creatingShelfRequestRef.current = true;
        try {
          const created = await createShelf(accessToken, {
            collection_id: data.collectionId,
            library_id: currentLibrary.id,
            section: currentLibrary.type === 'other' ? 'screen' : currentLibrary.type,
            ...values,
            show_in_main_watchlist: currentLibrary.type === 'screen' ? values.show_in_main_watchlist : false,
            position: (shelves.at(-1)?.position || 0) + 1000,
          });
          const row = created?.[0];
          if (!row?.id) throw new Error('The shelf was not created.');
          const createdShelf = {
            shelf_id: row.id,
            library_id: row.library_id,
            name: row.name,
            subtitle: row.subtitle || '',
            numbered: Boolean(row.is_numbered),
            queueList: Boolean(row.is_queue_list),
            readingList: Boolean(row.is_queue_list),
            section: row.section,
            position: row.position,
            deleted_at: row.deleted_at || null,
            required: Boolean(row.is_required),
            showInMainWatchlist: Boolean(row.show_in_main_watchlist),
            mainWatchlistPosition: row.main_watchlist_position ?? row.position,
          };
          onDataChange({
            ...data,
            loadedLibraries: [...new Set([...(data.loadedLibraries || []), currentLibrary.id])],
            mediaShelves: [...(data.mediaShelves || []).filter((shelf) => shelf.shelf_id !== row.id), createdShelf],
          });
          setCreatingShelf(false);
          try {
            await onLoadLibrary(currentLibrary.id, { fresh: true });
            notify('Shelf created.');
          } catch {
            notify('Shelf created and kept visible. The latest library data could not be reloaded yet.');
          }
        } catch (error) {
          notify(error?.code === '23505'
            ? 'That shelf could not be created. Names must be unique within this library.'
            : 'The shelf could not be created. Please try again.');
          throw error;
        } finally {
      ...
      {binOpen && <CollectionBinDrawer
        ...
        error={binError}
        onRetry={openBin}
        libraries={binLibraries}
        media={deletedMedia}
        shelves={deletedShelves}
        libraryName={(libraryId) => (data.libraries || []).find((library) => library.id === libraryId)?.name || binLibraries.find((library) => library.id === libraryId)?.name || 'Library'}
        onClose={() => setBinOpen(false)}
        onError={notify}
        onRestoreLibrary={async (library) => {
          await updateLibrary(accessToken, library.id, { deleted_at: null });
          setBinLibraries((rows) => rows.filter((row) => row.id !== library.id));
          await onLoadLibrary(library.id, { fresh: true });
          setBinOpen(false);
          notify(`${library.name} restored from Bin.`);
        }}
        onDeleteLibrary={async (library) => {
          if (library.protected) throw new Error('Default libraries cannot be permanently deleted.');
          const { shelfCount, itemCount } = await loadLibraryDeletionImpact({ libraryId: library.id, accessToken });
          requestConfirmation({
            title: `Permanently delete ${library.name}?`,
            message: `${library.name} contains ${shelfCount} ${shelfCount === 1 ? 'shelf' : 'shelves'} and ${itemCount} ${itemCount === 1 ? library.singular : library.plural}. The library and all of these nested records will be deleted. This cannot be undone.`,
            confirmLabel: 'Delete Library Permanently',
            tone: 'danger',
            optimistic: true,
            onConfirm: async () => {
              const previousData = data;
              const previousLibraries = binLibraries;
              const optimisticData = {
                ...data,
                libraries: (data.libraries || []).filter((row) => row.id !== library.id),
                mediaShelves: (data.mediaShelves || []).filter((row) => row.library_id !== library.id),
                media: (data.media || []).filter((row) => row.library_id !== library.id),
                loadedLibraries: (data.loadedLibraries || []).filter((id) => id !== library.id),
              };
              setBinLibraries((rows) => rows.filter((row) => row.id !== library.id));
              onDataChange(optimisticData);
              onInvalidateLibrary?.(library.id);
              try {
                const deleted = await permanentlyDeleteLibrary(accessToken, library.id);
                if (!deleted?.[0] || deleted[0].library_id !== library.id) throw new Error('Supabase did not delete the library.');
                notify(`${library.name} and its nested records were permanently deleted.`);
              } catch (error) {
        ...
        onOpenMedia={(itemId) => { setBinOpen(false); openMedia(itemId); }}
        onRestoreMedia={async (item) => { await setMediaDeleted(accessToken, item.database_id, false); await onLoadLibrary(item.library_id, { fresh: true, select: false }); notify(`${item.title} restored from Bin.`); }}
        onDeleteMedia={(item) => requestConfirmation({ title: `Permanently delete ${item.title}?`, message: 'This cannot be undone.', confirmLabel: 'Delete Permanently', tone: 'danger', optimistic: true, onConfirm: async () => { const previousData = data; const optimisticData = { ...data, media: data.media.filter((row) => row.database_id !== item.database_id) }; onDataChange(optimisticData); try { const deleted = await permanentlyDeleteMedia(accessToken, item.database_id); if (!deleted?.length) throw new Error('Supabase did not delete the media item.'); notify(`${item.title} permanently deleted.`); } catch (error) { onDataChange(previousData); notify(`${item.title} could not be deleted. It has been restored to the Bin.`); throw error; } } })}
      ...
      {bulkImportType && <BulkImportDialog type={bulkImportType} terminology={currentLibrary} shelves={shelves} onClose={() => setBulkImportType(null)} onImport={(shelfIds, rows) => {
...
function CollectionBinDrawer({ loading = false, error = '', onRetry, libraries = [], media, shelves, libraryName, onClose, onError, onRestoreLibrary, onDeleteLibrary, onOpenMedia, onRestoreMedia, onDeleteMedia, onRestoreShelf, onDeleteShelf }) {
  ...
  return createPortal(<div className="drawer-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="collection-bin-drawer">
      ...
      {loading && <div className="bin-loading" role="status"><RotateCw size={18} /><span><strong>Opening your complete Bin</strong><small>Your collection stays right where you left it.</small></span></div>}
      {!loading && error && <div className="bin-load-error" role="alert"><span><strong>The complete Bin could not be loaded</strong><small>{error}</small></span><button type="button" onClick={onRetry}>Try again</button></div>}
      {!loading && !error && !libraries.length && !media.length && !shelves.length && <div className="bin-empty"><Trash2 size={20} /><span>The Bin is empty.</span></div>}
      {libraries.length > 0 && <section className="bin-group"><header><span>LIBRARIES</span><small>{libraries.length}</small></header><div className="bin-list">
        {libraries.map((library) => <article className="bin-row-card shelf" key={library.id}><div className="bin-item-main"><span className="bin-shelf-mark"><BookOpen size={15} /></span><span><strong>{library.name}</strong><small>{LIBRARY_TYPE_DETAILS[library.type]?.label}</small></span></div><div className="bin-row-actions"><button type="button" disabled={busyId === library.id} onClick={() => run(library.id, () => onRestoreLibrary(library))}><RotateCw size={13} />Restore library</button>{!library.protected && <button className="permanent" type="button" disabled={busyId === library.id} onClick={() => run(library.id, () => onDeleteLibrary(library))}><Trash2 size={13} />Delete forever</button>}</div></article>)}
      </div></section>}
      ...
      <aside className="media-drawer public-drawer">
        ...
        <div className="drawer-layout">
          <div className="drawer-poster">
            ...
            <nav className="drawer-navigation" aria-label="Media shelf navigation">
              <button type="button" aria-label="Previous media item" title="Previous" disabled={!previousItem} onClick={onPrevious}><ChevronLeft /></button>
              <button type="button" aria-label="Next media item" title="Next" disabled={!nextItem} onClick={onNext}><ChevronRight /></button>
            </nav>
          </div>
...
function WatchlistRequestDialog({ request, shelves, busy, onRespond, onDismiss }) {
  ...
  const RequestIcon = isStampRequest ? Stamp : ListOrdered;
...
function WatchlistRequestListDialog({ requests, onOpen, onClose }) {
  ...
  return <div className="modal-layer editor-layer watchlist-request-list-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="media-edit-dialog watchlist-request-list" role="dialog" aria-modal="true" aria-labelledby="watchlist-request-list-title">
      ...
      <header className="watchlist-request-list-hero">
        <span className="watchlist-request-emblem"><Bell size={22} /></span>
        <span><small>SIGNED IN · WATCHLIST</small><h2 id="watchlist-request-list-title">{requests.length === 1 ? 'One request needs your attention' : `${requests.length} requests need your attention`}</h2><p>Choose a title to review the request.</p></span>
      </header>
      <div className="watchlist-request-list-items">{requests.map((request) => {
        const isStampRequest = request.request_type === 'priority_stamp_removal';
        return <button type="button" className="watchlist-request-list-item" key={request.id} onClick={() => onOpen(request)}>
          <span className={cls('watchlist-request-type-icon', isStampRequest ? 'stamp' : 'move')}>{isStampRequest ? <Stamp size={16} /> : <ListOrdered size={16} />}</span>
          <span><small>{isStampRequest ? 'PRIORITY STAMP' : 'WATCHED ITEM'}</small><strong>{request.media_title}</strong><span>{watchlistRequestMessage(request)}</span></span>
...
function ShelfTransferDialog({ transfer, sourceLibrary, sourceOwnerName, ownerData, loadDestinations, ownCollection, onClose, onConfirm, onViewResult }) {
  ...
  useEffect(() => {
    ...
    const destinationNames = (destinationData?.mediaShelves || [])
      .filter((shelf) => !shelf.deleted_at && shelf.library_id === destinationLibraryId)
...
function CreateShelfDialog({ section, terminology, onClose, onSave }) {
  ...
  const [saving, setSaving] = useState(false);
  return <div className="modal-layer editor-layer" onMouseDown={(event) => event.target === event.currentTarget && !saving && onClose()}><form className="media-edit-dialog shelf-edit-dialog create-shelf-dialog" onSubmit={async (event) => { event.preventDefault(); if (!name.trim() || saving) return; setSaving(true); try { await onSave({ name: name.trim(), subtitle: subtitle.trim() || null, is_numbered: numbered, ...(section === 'screen' ? { show_in_main_watchlist: mainWatchlist } : {}) }); } catch { setSaving(false); } }}>
    ...
    <small className="character-count">{subtitle.length} / 180</small>
    <div className="dialog-actions"><button className="text-button" type="button" onClick={onClose} disabled={saving}>Cancel</button><Button type="submit" icon={Plus} disabled={!name.trim() || saving}>{saving ? 'Creating…' : 'Create Shelf'}</Button></div>
  </form></div>;
}
...
function BulkImportDialog({ type, terminology, shelves, onClose, onImport }) {
  useEscape(onClose);
  const [text, setText] = useState('');
  const [shelfIds, setShelfIds] = useState(() => shelves[0]?.shelf_id ? [shelves[0].shelf_id] : []);
  const importTerms = bulkImportTerminology(type, terminology);
  ...
  return <div className="modal-layer editor-layer"><form className="media-edit-dialog bulk-import-dialog" onSubmit={(event) => { event.preventDefault(); if (!shelfIds.length || !items.length) return; onImport(shelfIds, items); }}>
    <button className="close" type="button" onClick={onClose} aria-label="Close bulk import"><X /></button><span className="eyebrow">BULK IMPORT</span><h2>Bulk Import {importTerms.heading}</h2><p className="dialog-intro">Add one title per line, then choose every shelf it belongs on.</p>
    ...
    <div className="dialog-actions"><button type="button" className="text-button" onClick={onClose}>Cancel</button><Button type="submit" icon={Plus} disabled={!items.length || !shelfIds.length}>{items.length ? `Import ${items.length} ${items.length === 1 ? importTerms.singular : importTerms.plural}` : 'Import'}</Button></div>
  </form></div>;
}
